...
class FormCuenta : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val nombreUsuarioSeleccionado = intent.getStringExtra("nombreUsuarioSeleccionado")
        ...
        val codigoCuenta = intent.getStringExtra("positionCuentaSelected")
        if (codigoCuenta == "") {
            fireStoreCuenta.consultarCuenta(codigoCuenta!!) {
                inputNombreCuenta.setText(it.nombreCuenta)
                inputCantidad.setText(it.cantidad.toString())
                inputCreacionCuenta.setText(it.fechaCreacion.toString())
                inputCaducado.setText(it.esCaducada.toString())
            }
        }
        ...
        if (codigoCuenta == "") {
            ...
            btnCrear.setOnClickListener {
                try {
                    nombreCuenta = inputNombreCuenta.text.toString()
                    cantidad = inputCantidad.text.toString().toLong()
                    fechaCreacion = LocalDate.parse(inputCreacionCuenta.text.toString())
                    esCaducado = inputCaducado.text.toString().toBoolean()

                    fireStoreCuenta.crearCuenta(
                        Cuenta(
                            nombreCuenta,
                            cantidad,
                            fechaCreacion,
